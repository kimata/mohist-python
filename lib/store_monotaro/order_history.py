...
def insert_table_header(handle, sheet, row, style):
    store_monotaro.handle.set_status(handle, "テーブルのヘッダを設定しています...")
...
def insert_table_cell_image(handle, sheet, row, col, item):
    thumb_path = store_monotaro.handle.get_thumb_path(handle, item)
...
def setting_table_view(handle, sheet, row_last):
    store_monotaro.handle.set_status(handle, "テーブルの表示設定しています...")
...
def insert_sum_row(handle, sheet, row_last, style):
    ...
    store_monotaro.handle.set_status(handle, "集計行を挿入しています...")
...
def generate_list_sheet(handle, book):
    ...
    store_monotaro.handle.set_status(handle, "購入商品の記載をしています...")

    row += 1
    for item in item_list:
        sheet.row_dimensions[row].height = TABLE_HEADER["row"]["height"]
        insert_table_item(handle, sheet, row, item, style)
        store_monotaro.handle.get_progress_bar(handle, STATUS_INSERT_ITEM).update()
        row += 1
...
def generate_table_excel(handle, excel_file):
    ...
    book._named_styles["Normal"].font = store_monotaro.handle.get_excel_font(handle)

    store_monotaro.handle.get_progress_bar(handle, STATUS_ALL).update()

    generate_list_sheet(handle, book)

    store_monotaro.handle.set_status(handle, "エクセルファイルを書き出しています...")
    ...
    store_monotaro.handle.set_status(handle, "完了しました！")
